...
def sdf_new(model_output, gt):
    ...
    # Wherever boundary_values is not equal to zero, we interpret it as a boundary constraint.
    sdf_constraint = torch.where(gt_sdf == 0, pred_sdf, torch.zeros_like(pred_sdf))
    sdf_near_constraint = torch.where(gt_sdf == 0.025, pred_sdf, torch.zeros_like(pred_sdf))
    sdf_far_constraint = torch.where(gt_sdf == -0.0125, pred_sdf, torch.zeros_like(pred_sdf))
    inter_constraint = torch.where(gt_sdf == 0, torch.zeros_like(pred_sdf), torch.exp(-1e1 * torch.abs(pred_sdf)))
    normal_constraint = torch.where(gt_sdf == 0, 1 - F.cosine_similarity(gradient, gt_normals, dim=-1)[..., None],
                                    torch.zeros_like(gradient[..., :1]))
    grad_constraint = torch.abs(gradient.norm(dim=-1) - 1)
    # Exp      # Lapl
    # -----------------
    return {'sdf': torch.abs(sdf_constraint).mean() * 2e3 + torch.abs(sdf_near_constraint-0.025).mean() * 5e2 + torch.abs(sdf_far_constraint+0.0125).mean() * 5e2,  # 1e4      # 3e3
            'inter': inter_constraint.mean() * 1e-9,  # 1e2                   # 1e3
            'normal_constraint': normal_constraint.mean() * 3e3,  # 1e2
            'grad_constraint': grad_constraint.mean() * 3e2}  # 1e1      # 5e1
